...
UDHR comparisons use a versioned UTF-8 snapshot generated from the Unicode UDHR Project at a pinned commit. Corpus v2 stores all 501 available stage-4 records locally, representing 431 ISO 639-3 language codes. Of those records, 465 link to an OHCHR translation and 36 retain a separate `unicode-complete` provenance tier. The audit accepts 496 records across 426 language codes for aligned comparison; five upstream XML records are preserved but blocked because they do not contain Articles 1–30 in full. The current interface continues to expose its reviewed 61-record subset until the grouped language-browser release.
