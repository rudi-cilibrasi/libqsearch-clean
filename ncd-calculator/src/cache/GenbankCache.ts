...
export class GenbankCache {
    ...
    async mergeSuggestionsAndGet(
        ...
        const key = this.createSearchKey(mode, term);
        const existingCache = await this.localStorageCache.getCacheEntry(CACHE_NAMES.SUGGESTIONS, "") as SuggestionsCache | null;
        const existingEntry = (existingCache?.[key] as SuggestionCacheEntry | undefined) || {
            accessionIds: [],
        ...
        const existingDetails = await this.localStorageCache.getCacheEntry(CACHE_NAMES.SUGGESTION_DETAILS, "") || {};
