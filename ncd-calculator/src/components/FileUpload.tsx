...
export const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const [effectiveAlgorithm, setEffectiveAlgorithm] = useState<'gzip' | 'lzma'>('gzip');
  ...
  const determineEffectiveAlgorithm = useCallback((fileInfos: FileInfo[]): 'gzip' | 'lzma' => {
    ...
    if (selectedCompression === 'auto') {
      return largestPairSize <= GZIP_MAX_WINDOW ? 'gzip' : 'lzma';
    }
    return selectedCompression as 'gzip' | 'lzma';
  }, [selectedCompression]);
        ...
        <div className="flex-1">
          ...
          >
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl flex items-start gap-2 max-w-lg mb-4">
              <FileSearch className="text-blue-600 flex-shrink-0 mt-1" size={20} />
              <div className="space-y-2">
                <div>
                  ...
                  <p className="text-blue-800 text-xs leading-relaxed">
                    Upload any files and watch as we reveal their hidden connections. Perfect for:
                  </p>
                </div>
