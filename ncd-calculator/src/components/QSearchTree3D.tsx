...
export interface QSearchTree3DProps {
    ...
    ncdMatrix?: number[][]; // Add NCD matrix for proper distance-based clustering
    labels?: string[]; // Add labels to map matrix indices to node labels
}
...
export const QSearchTree3D: React.FC<QSearchTree3DProps> = ({data, labelManager, ncdMatrix, labels}) => {
    ...
    const renderVisualization = () => {
        ...
        return (
          ...
          >
              <PerspectiveCamera
                makeDefault
                position={[scaleFactor * 80, scaleFactor * 20, scaleFactor * 80]}
                fov={60}
                near={1}
                far={1000}
              />
...
const QSearchTree: React.FC<QSearchTreeProps> = ({data, scaleFactor, theme, labelManager, ncdMatrix, labels}) => {
      ...
      nodes: QTreeNode[]
        ...
        const positionNode = (
          ...
          level: number
            ...
            const y = Math.sin(level) * radius + (Math.random() - 0.5) * randOffset * radius - spread * 0.5; // Lower the tree by offsetting Y
