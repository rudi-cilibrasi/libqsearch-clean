...
export const FastaSearchSuggestion = ({
  ...
  useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        const suggestions = await genbankSearchService.getSuggestions(normalizedSearchTerm, count + 1);
  ...
  }, [searchTerm]); // Removed selectedIds from dependency array
  ...
  const setLastLocalPageCountObj = (searchTerm, count) => {
    const obj = getLastLocalPageCountObj(searchTerm);
    if (!obj) {
      setLocalPageCount({
        ...localPageCount,
        [searchTerm]: {
